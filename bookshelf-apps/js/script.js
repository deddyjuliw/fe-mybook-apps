...
function make_card_book({ id, title, author, year, isComplete }) {
    ...
    author_element.innerText = `Writer : ${author}`;

    const year_element = document.createElement('p');
    year_element.innerText = `Year : ${year}`;

    const btn_blue = document.createElement('button');
    btn_blue.classList.add('blue');
    ...
    btn_red.addEventListener('click', function() {
        const confirm_remove = confirm("Are you sure want to delete it?");
        if (confirm_remove === true) return handler_remove_book(id);
    })

    const div_element = document.createElement('div');
    div_element.classList.add('action');
    div_element.append(btn_blue, btn_red);
    ...
    if (isComplete) {
        btn_blue.innerHTML = `<i class="uil uil-book-alt"></i>`;
        ...
        btn_blue.innerHTML = `<i class="uil uil-book-open"></i>`;
...
function add_book() {
    ...
    if (to_book.push(books)) {
        title_bar.innerText = "SAVED!";
        title_bar.classList.add('success');
    }
...
function load_data_from_storage() {
    ...
    let data_object = JSON.parse(serialize_data);

    if (data_object !== null) {
        for (const books of data_object) {
            to_book.push(books);
        }
    }
    document.dispatchEvent(new Event(RENDER_BOOK));
